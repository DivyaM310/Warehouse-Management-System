        ...
        private void Save_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                for (int i = 0; i <= ListView1.Items.Count - 1; i++)
                {
                    con = new SqlConnection(cs.DBConn);
                    con.Open();
                    string cb2 = "update dbo.stock set Quantity = Quantity - " + ListView1.Items[i].SubItems[4].Text + " where ProductID= '" + ListView1.Items[i].SubItems[1].Text + "'";
                    cmd2 = new SqlCommand(cb2);
                    cmd2.Connection = con;
